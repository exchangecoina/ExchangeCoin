...
#if defined(NDEBUG)
# error "exchangecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf522787801b34ebb86dc17099e23feee2d83195f018293774cf061ef9aa6d7ad");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // exchangecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "exchangecoin Signed Message:\n";
...
{
    // exchangecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // exchangecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // exchangecoin: 5 mins
static const int64 nTargetSpacing = 1 * 30; // exchangecoin: 30 secs
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // exchangecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // exchangecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf522787801b34ebb86dc17099e23feee2d83195f018293774cf061ef9aa6d7ad");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "bitcoin hit 2800";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1496806203;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1237713;
        ...
        assert(block.hashMerkleRoot == uint256("0xfef93e9654e543cf419f1d8e9b547a79a85118a7b8b6de0d9d6b33979bc601f6"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // exchangecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("exchangecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("exchangecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static exchangecoinMiner(CWallet *pwallet)
{
    printf("exchangecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("exchangecoin-miner");
    ...
    try { loop {
        ...
        printf("Running exchangecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("exchangecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&exchangecoinMiner, pwallet));
}
